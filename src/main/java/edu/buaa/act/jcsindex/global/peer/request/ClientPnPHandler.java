/*
 * @(#) ClientPnPHandler.java 1.0 2006-10-15
 * 
...
//import buaa.act.jointcloudstorage.peer.*;

/**
 * A particular <code>UDPHandler</code> is responsible for processing 
 * <code>{@link MsgType#PING}</code>,
 * <code>{@link MsgType#PONG}</code>, and
 * <code>{@link MsgType#TROUBLESHOOT}</code> messages
 * from bootstrapper and server peers.
 * 
 * @author Xu Linhao
 * @version 1.0 2006-9-26
 * 
 * @see UDPHandler
 * @see ClientPnPReceiver
 */

public class ClientPnPHandler extends UDPHandler
{
	 ...
	 */
	public ClientPnPHandler(DatagramSocket ds) 
	{
		super(ds);
	}
	 ...
	 * <ul>
	 * <li>case 1: if receive a ping message from its server peer, 
	 * then return a pong message to the server peer.</li>
	 * <li>case 2: if receive a pong message from its server peer, 
	 * then notify the UDPServer that the server peer is still online.</li>
	 * <li>case 3: if receive a troubleshoot message from its server peer, 
	 * then leave the system.</li>
	 * </ul>
	 */
...
//				DatagramPacket pong = this.pong(PeerType.CLIENTPEER, session.getUserID());
...
//			info = this.parsePong(buf, 4);
			...
			Object[] list;	// get a copy of listeners
			synchronized (this)
			{
