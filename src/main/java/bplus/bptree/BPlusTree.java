...
//    }
    ...
    public List<Long> ranges() {
        ...
        if (root != null) {
            for (int i = 0; i < root.keyArray.size(); i++) {
                res.add(root.keyArray.get(i));
            }
        }
        return res;
    }
    ...
    private boolean findRange(TreeNode n, RangePosition rp, int ind) {
        if (n.isLeaf()) {
            // 叶子结点
            if (ind != rp.level) {
                return false;
            ...
            int i = rp.indexs.get(ind);
            if (ind > rp.level) {
                // 当ind > rp.level时，返回false
                return false;
            } else if (ind == rp.level) {
                // 当 ind == rp.level时，说明到了正确的层次，可以取出相应的范围
                if (i == 0) {
                    rp.left = 0;
                    rp.right = n.keyArray.get(0);
                } else if (i == n.keyArray.size()) {
                    rp.left = n.keyArray.get(i - 1);
                    rp.right = Long.MAX_VALUE;
                } else {
                    rp.left = n.keyArray.get(i - 1);
                    rp.right = n.keyArray.get(i);
                }
                return true;
            } else {
                TreeInternalNode internalNode = (TreeInternalNode ) n;
                try {
                    TreeNode child = readNode(internalNode.getPointerAt(i));
                    return findRange(child, rp, ind + 1);
    ...
    public List<RangePosition> expandRange(TreeNode n, RangePosition rp, int ind) {
        if (n.isLeaf()) {
            ...
            if (ind > rp.level) {
                ...
                List<RangePosition> res = new ArrayList<>();
                long left = 0;
                for (int j = 0; j < n.keyArray.size(); j++) {
                    List<Integer> indexs = new ArrayList<>(rp.indexs);
                    indexs.add(j);
                    res.add(new RangePosition(rp.level + 1, indexs, left, n.keyArray.get(j)));
                    left = n.keyArray.get(j);
                }
                List<Integer> indexs = new ArrayList<>(rp.indexs);
                ...
                try {
                    TreeNode child = readNode(internalNode.getPointerAt(i));
                    return expandRange(child, rp, ind + 1);
                } catch (IOException ex) {
                    ex.printStackTrace();
                    return null;
                }
